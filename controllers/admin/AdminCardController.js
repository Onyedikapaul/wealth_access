...
AdminCardRouter.patch("/cards/:id/status", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: `Card status updated to "${status}".`, card });
  } catch (error) {
...
AdminCardRouter.patch("/cards/:id", async (req, res) => {
  try {
    ...
    if (card_holder_name !== undefined)
      update.card_holder_name = card_holder_name;
    ...
    const card = await CardModels.findByIdAndUpdate(id, update, {
      new: true,
    ...
    return res
      .status(200)
      .json({ message: "Card updated successfully.", card });
  } catch (error) {
...
AdminCardRouter.delete("/cards/:id", async (req, res) => {
  try {
    const { id } = req.params;

    const card = await CardModels.findByIdAndDelete(id);
    if (!card) return res.status(404).json({ message: "Card not found." });
