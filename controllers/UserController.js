...
async function sendVerificationEmail(email, name, otp) {
  ...
  console.log("sendVerificationEmail sent:", data);
}
...
export const loginUser = async (req, res) => {
  try {
    ...
    if (error) {
      console.error("loginUser Resend error:", JSON.stringify(error, null, 2));
      return res
        .status(500)
        .json({ message: "Failed to send verification email." });
    }

    console.log("loginUser OTP email sent:", data);
...
async function sendWelcomeEmail(user, accountNumber) {
  ...
  if (error) {
    console.error(
      "sendWelcomeEmail Resend error:",
      JSON.stringify(error, null, 2),
    );
    throw new Error("Failed to send welcome email");
  }

  console.log("sendWelcomeEmail sent:", data);
}
...
export const resendLoginOTP = async (req, res) => {
  try {
    ...
    console.log("resendLoginOTP email sent:", data);
