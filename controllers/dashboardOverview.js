...
// -------- Helpers --------
const isDone = (status) =>
  ["completed", "successful", "complete", "success", "approved"].includes(
    (status || "").toLowerCase()
  );
...
const normalizeTransfer = (doc, source) => {
  ...
  return {
    ...
    activity: isLocal
      ...
      : doc.method || "International Wire",
    method: isLocal ? "local" : doc.method,
    type: (doc.type || "debit").toLowerCase().trim(), // ✅ normalize casing
    amount: Number(doc.amount || 0),
    status: (doc.status || "pending").toLowerCase(),
    date: doc.createdAt || doc.processedAt || new Date(),
    source,
...
// GET /api/dashboard/spending-limit
DashboardOverviewRouter.get("/spending-limit", checkAuth, async (req, res) => {
  try {
    const userId = req.user?._id || req.user?.id || req.userId;
    ...
    // this month's completed debits = spending
    const monthStart = startOfMonth();
    const spent = all
      .filter(
        (t) =>
    ...
    // limit — use user's monthlyLimit or fallback to 2x transactionLimit
    const transactionLimit = Number(user.transactionLimit || 500000);
    const limit = Number(user.monthlyLimit || transactionLimit);

    res.json({ spent, limit });
  } catch (err) {
    console.error("[/api/dashboard/spending-limit]", err);
    res.status(500).json({ error: "Failed to load spending limit" });
  }
});
