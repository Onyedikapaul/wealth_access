...
function getIntlSymbol() {
  return window._userProfile?.currency?.symbol ?? "$";
}
function getIntlCode() {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.submitTransfer = async function (alpineData) {
    ...
    try {
      ...
      if (data.success) {
        ...
        fields.forEach((f) => {
          if (f in alpineData) alpineData[f] = "";
        });
  ...
  function buildDetails(method, body) {
    switch (method) {
      ...
      case "Wise Transfer":
        return {
          fullName: body.wiseFullName,
          email: body.wiseEmail,
          country: body.wiseCountry,
        };
      case "Skrill":
      ...
      case "Zelle":
        return {
          email: body.zelleEmail,
          phone: body.zellePhone,
          fullName: body.zelleName,
        };
      case "Cash App":
        return { cashtag: body.cashAppTag, fullName: body.cashAppFullName };
      case "Revolut":
        return {
          fullName: body.revolutFullName,
          email: body.revolutEmail,
          phone: body.revolutPhone,
        };
      case "Alipay":
  ...
  window.showIntlReceiptModal = function (txn) {
    ...
    switch (txn.method) {
      ...
      case "Wise Transfer":
        detailRows = [
          optRow("Full Name", d.fullName),
          optRow("Email", d.email),
          optRow("Country", d.country),
        ].join("");
        break;
      case "Skrill":
      ...
      case "Venmo":
        detailRows = [
          optRow("Username", d.username),
          optRow("Phone", d.phone),
        ].join("");
        break;
      case "Zelle":
        detailRows = [
      ...
      case "Alipay":
        detailRows = [
          optRow("Alipay ID", d.alipayId),
          optRow("Full Name", d.fullName),
        ].join("");
        break;
      case "WeChat Pay":
        detailRows = [
          optRow("WeChat ID", d.wechatId),
          optRow("Full Name", d.fullName),
        ].join("");
        break;
    }
